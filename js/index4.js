...
var getArrivalTimes = function() {
    ...
    fetch(url)
    //http://api.bart.gov/api/etd.aspx?cmd=etd&orig=12th&key=ZSBS-PM93-9UET-DWE9&json=y
        .then (function(response) {
            return response.json()
        })
        .then (function(json)  {
            ...
            var newEstimate = estimates[i].estimate
                ...
                for(var j=0; j < newEstimate.length; j++ ){
                    console.log(newEstimate[j].minutes)
                    depart.innerHTML += 'minutes' + newEstimate[j].minutes+" " + " platform: "+ newEstimate[j].platform + "<br>"
